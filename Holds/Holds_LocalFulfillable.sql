...
WHERE i.location_code ~ '^art' AND i.is_available_at_library = 'true' AND i.item_status_code = '-'
)
...
FROM
sierra_view.hold h
WHERE h.pickup_location_code ~ '^art'
)hold_num
ON
h.id = hold_num.id
JOIN
...
ON
h.record_id = ir.id AND ir.location_code ~ '^art' AND ir.is_available_at_library = 'true' AND ir.item_status_code = '-'
JOIN
sierra_view.bib_record_item_record_link l
ON
